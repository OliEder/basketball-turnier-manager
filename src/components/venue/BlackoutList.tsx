    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 items-end flex-wrap">
        <div className="space-y-1">
          ...
          <Input id="blackout-start" type="time" value={newBlackout.start} onChange={e => setNewBlackout(p => ({ ...p, start: e.target.value }))} className="w-32" />
        </div>
        <div className="space-y-1">
          <Label htmlFor="blackout-end">Bis</Label>
          <Input id="blackout-end" type="time" value={newBlackout.end} onChange={e => setNewBlackout(p => ({ ...p, end: e.target.value }))} className="w-32" />
        </div>
        <div className="space-y-1 flex-1">
          <Label htmlFor="blackout-reason">Grund (optional)</Label>
          <Input id="blackout-reason" value={newBlackout.reason ?? ''} onChange={e => setNewBlackout(p => ({ ...p, reason: e.target.value }))} placeholder="z.B. Mittagspause" />
        </div>
